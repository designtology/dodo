...
<table id="projects" class="table table-striped table-bordered table-dark dataTable no-footer" role="grid" aria-describedby="animalsTable_info">
  <thead>
    <tr role="row">
      ...
      <td class="sorting" data-order="desc" tabindex="0" rowspan="1" colspan="1" aria-label="hours_worked" style="width: 80px;">Abgearbeitet</td>      
...
if(mysqli_num_rows($result) > 0){
  while($row = mysqli_fetch_assoc($result)){
    ...
    if($row[project_id] != $temp){
      ...
      $deadline_formated = date( 'd.m.Y', $deadline_row );        $worked_hours_full = 0;
      ...
      ?>
        ...
        <tr role="row" class="table_row_new_line">
          ...
          <td></td>
          <td></td>
          <td><b>Gesamt: <?php echo $hours; ?> Std</b></td>
    ...
    <tr role="row positions">
      ...
      <td><?php echo number_format($worked_hours / human2seconds("00:".$row[position_hours] . ":00:00") * 100,0,'',''); ?> %</td>
